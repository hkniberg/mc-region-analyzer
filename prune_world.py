...
def build_masks(db_path, threshold):
    keep, known = {}, {}
    ...
    for dim, cx, cz, inh in cur:
        ...
        bit = 1 << idx
        k = (dim, rx, rz)
        known[k] = known.get(k, 0) | bit
        # keep when at/above threshold, or when we couldn't read it (NULL)
        if inh is None or inh >= threshold:
            keep[k] = keep.get(k, 0) | bit
        n += 1
    db.close()
    return keep, known, n
...
def main():
    ...
    print(f"keep threshold: inhabited_ticks >= {args.min_ticks} "
          f"({args.min_ticks/1200:.0f} min)   {'DRY-RUN' if DRY_RUN else 'WRITE'}")
    KEEP, KNOWN, nrows = build_masks(args.db, args.min_ticks)
